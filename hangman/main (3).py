...
for current_answer_character in answer_characters:
  if re.search("^[A-Z]$",current_answer_characters): answer_guesses.append(False)
else:
...
for answer_index in range(len(answer_characters)):
  ...
  if letter not in guessed_letter and len(letter) == 1 and re.search("^[A-Z]$", letter):
...
.   for current_guessed_letter in guessed_letters:
    ...
    if letter in answer_charcaters:
    #letter is in the puzzle
      for current_answer_characters in range (len(answer_characters)): 
...
if num_of_incorrect_guesses < 5:
  ...
  print("Dorry you ran out og guesses.")
